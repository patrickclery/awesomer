...
export async function getTrendingLists() {
  return fetchApi<SingleResponse<TrendingList[]>>('/trending/lists');
}
...
export async function getLastSnapshotDate() {
  return fetchApi<SingleResponse<{ lastSnapshotDate: string | null }>>('/sync/static/last-snapshot');
}
