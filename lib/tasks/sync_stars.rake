...
namespace :sync do
  ...
  task stars: :environment do
    ...
    items_without_stars.find_in_batches(batch_size: batch_size).with_index do |batch, batch_index|
      ...
      batch.each_with_index do |item, index|
        ...
        begin
          ...
          # Progress indicator
          if updated % 10 == 0
            puts "  Progress: #{updated}/#{total} items updated"
          end
  ...
  task :stars_for_list, [:repo] => :environment do |t, args|
    repo = args[:repo]
