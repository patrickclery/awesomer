...
VCR.configure do |config|
  ...
  config.filter_sensitive_data('[GITHUB_API_KEY]') { ENV.fetch('GITHUB_API_KEY', nil) }
  ...
  module Support
    module VCR
      ...
      def vcr(directory, name, **additional_options, &block)
        ...
        options[:record] = ENV['VCR_RECORD_MODE'].to_sym if ENV['VCR_RECORD_MODE'].present?
